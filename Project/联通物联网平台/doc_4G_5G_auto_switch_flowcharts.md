...
- [4G/5G自动切换功能 - 详细流程图](#4g5g自动切换功能---详细流程图)
  ...
  - [7. 完整状态机总览](#7-完整状态机总览)
    - [7.1 简化版状态机（推荐Obsidian查看）](#71-简化版状态机推荐obsidian查看)
    - [7.2 分层状态机（详细版）](#72-分层状态机详细版)
      - [主状态层](#主状态层)
      - [5G监控子状态](#5g监控子状态)
      - [4G监控子状态](#4g监控子状态)
    - [7.3 完整状态机（仅供参考）](#73-完整状态机仅供参考)
    - [7.4 线性流程图版本（最清晰）](#74-线性流程图版本最清晰)
  - [8. 异常处理流程](#8-异常处理流程)
...
### 7.1 简化版状态机（推荐Obsidian查看）

```mermaid
graph LR
    Start([初始化]) --> 5G[5G监控]
    
    5G -->|质量差+达到keep_time| Switch4G[切换到4G]
    Switch4G -->|附网+检测通过| 4G[4G监控]
    Switch4G -->|失败| 5G
    
    4G -->|质量差| Switch5G_Bad[切回5G]
    4G -->|质量好+达到keep_time| Switch5G_Try[尝试5G]
    Switch5G_Bad --> 5G
    Switch5G_Try --> 5G
    ...
    style 5G fill:#ccffcc
    style 4G fill:#ffeecc
    style Switch4G fill:#ffcccc
    style Switch5G_Bad fill:#ccccff
    style Switch5G_Try fill:#e1e1ff
```

**核心逻辑**：
- 🟢 **5G监控**：主状态，质量差且达到保持时间→切4G
- 🟠 **4G监控**：备用状态，质量差→立即切回5G；质量好→定期尝试5G
- 🔄 **切换保护**：4G切换需附网检测，失败则回退5G
...
<details>
<summary>点击展开完整版状态机（复杂，不推荐Obsidian直接查看）</summary>
...
**推荐使用方式**：
- 📱 **Obsidian查看**：使用 7.1 简化版或 7.4 线性流程图
- 📄 **文档阅读**：使用 7.2 分层状态机理解细节
- 🔍 **完整参考**：需要时展开 7.3 完整版
